...
class DetrTrainer:
    ...
    def create_dataset(self):
        train_dataset = CocoDetection(self.image_path, self.train_json_annot_path, self.val_json_annot_path, feature_extractor = self.feature_extractor)
        val_dataset = CocoDetection(self.image_path, self.train_json_annot_path, self.val_json_annot_path, feature_extractor = self.feature_extractor, train=False)
        test_dataset = CocoDetection(self.image_path, self.train_json_annot_path, self.test_json_annot_path, feature_extractor = self.feature_extractor, train=False)
    ...
    def train(self, train_dataset, val_dataset):
        ...
        trainer = Trainer(max_epochs = self.max_epochs, gradient_clip_val = self.gradient_clip_val, logger=logger, callbacks=[self.early_stop])
        trainer.fit(model)
